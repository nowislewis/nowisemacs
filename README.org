...
*** Add packages
1. Add a new submodule to the =lib= directory.
2. Run =make lib/package=.

*** Remove packages
1. Remove the submodule from the =.gitmodules= file and the =lib= directory.
...
1. Manually manage packages with git submodules (=git submodule add URL lib/package-name=).
