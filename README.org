...
3. =cd ~/.emacs.d & make init= ;; download all packages
4. =make -j8 build= ;; build all packages
   1. (optional) =make native= -j8 ;; if you build emacs with native-comp, use this command to build all packages ahead-of-time.
5. =emacs= ;; Launch emacs, the download, installation and configuration of packages will be done automatically.

 *** Add packages
 1. =M-x capsule-add-package= and enter the git URL (or manually add git submodule to =lib= directory).
 2. Run =make lib/package= to build.

 *** Remove packages
 1. =M-x capsule-remove-package= and select the package (or manually remove from =.gitmodules= and =lib= directory).

*** Update packages
1. =make update= ;; get all packages update latest
2. =make -j8 build= ;; build packages
 ...
 *** Everyday use(a summary)
 ...
 1. =M-x capsule-add-package= / =M-x capsule-remove-package= for package management.
 2. =make update= for updates. After update, run =make -j8 build= to recompile.
 3. =make init-org= if you only changed the =init.org= file.
 4. If you modify a local package under lib such as =foo=, run =make lib/foo= to recompile only that package.

*Package management is done via git submodules directly. Use =make -j8 build= for parallel compilation (faster).*
