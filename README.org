...
** How to customize
Every time you want to change some configs, do that in =~/.emacs.d/init.org= (eg, write down =<s= and then use =tab= to get a expanded src code) and run =org-babel-tangle= or =make tangle-init=. This will overwrite =~/.emacs.d/init.el= and =early-init.el=. Nowisemacs use this method to guarantee the =init.el= will not be changed by mistake =custom-group= or something else.
