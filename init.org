...
(transient-define-prefix apps-leader-map()
  ...
  [["agenda"
   ...
   ["capture"
    ...
    ;; ("C" "clarify entry (AI refile)" nowis/gtd-clarify-entry)
    ;; ("W" "weekly review (AI)" nowis/gtd-weekly-review)
    ]
   ["life"
...
;; chinese font
(setq lewis-chinese-font
      (nowis/first-available-font '("Maple Mono SC NF"
                                    "Unifont"
                                    "Sarasa Mono SC Nerd"
                                    "Microsoft Yahei")))
...
;; fido-vertical-mode 会自动开启 fido-mode + icomplete-vertical-mode
(fido-vertical-mode 1)

;; 在更高的 depth 覆盖它，以继续使用 orderless
(defun lewis/fido-use-orderless ()
  "Override fido-mode's flex completion-styles with orderless."
  (when (and fido-mode (icomplete-simple-completing-p))
    (setq-local completion-styles '(orderless basic)
                completion-category-defaults nil
                completion-category-overrides '((file (styles partial-completion))))))
(add-hook 'minibuffer-setup-hook #'lewis/fido-use-orderless 99)

;; 候选行超出屏幕宽度时截断而非换行
(defun lewis/minibuffer-truncate-lines ()
  (setq-local truncate-lines t))
(add-hook 'minibuffer-setup-hook #'lewis/minibuffer-truncate-lines 99)

;; 调整候选列表高度（fido 通过 max-mini-window-height 控制）
...
(setup ghostel
  ...
  (:when-loaded
    ;; ;; delete-backward-char (and variants) should send DEL (\x7f) to the
    ;; ;; terminal rather than trying to modify the read-only buffer.
    ;; (keymap-set ghostel-mode-map "<remap> <delete-backward-char>" #'ghostel--send-event)
    ;; (keymap-set ghostel-mode-map "<remap> <backward-delete-char>" #'ghostel--send-event)
    ;; (keymap-set ghostel-mode-map "<remap> <backward-delete-char-untabify>" #'ghostel--send-event)
...
(setq text-mode-ispell-word-completion nil)
  ...
  (dolist (f '(cape-file cape-dict cape-elisp-block cape-dabbrev-min-2))
...
(setup appt
  ...
  (with-eval-after-load 'org-agenda
    ...
    ;; 更新agenda时，同步appt（t = 先清空再重建，防止旧条目残留）
  ...
  (:when-loaded
    (setq-default gptel-backend
                  (gptel-make-anthropic "momenta"
                    ...
                    :key 'gptel-api-key
                    :stream t
                    :models '(claude-sonnet-4.6 claude-haiku-4.5 gemini-3.1-pro qwen3.6-plus-anthropic qwen3.6-plus)))
    ...
    (defun nowis/gpt-save--slug ()
      "Return a sanitized 10-char slug from the first non-empty line of body text.
 Skips gptel system prompt (lines starting with @@) and blank lines."
      (save-excursion
        (goto-char (point-min))
        (let ((text ""))
          (while (and (= (length text) 0)
            ...
            (let ((line (string-trim (thing-at-point 'line t))))
              (unless (or (string-empty-p line)
                          ...
                          (string-prefix-p "#+" line))
                (setq text (substring line 0 (min 10 (length line)))))
              (forward-line 1)))
          (if (string-empty-p text)
              "untitled"
            ;; replace filesystem-unsafe chars with hyphen
            (replace-regexp-in-string "[^[:alnum:]\u4e00-\u9fff\u3040-\u30ff]+"
                                      "-" text)))))
      ...
      (when (and (bound-and-true-p gptel-mode)
        ...
        (if buffer-file-name
          ...
          (let* ((ts   (format-time-string "%Y%m%dT%H%M%S"))
                 (slug (nowis/gpt-save--slug))
                 (dir  (file-name-as-directory nowis/gpt-save-directory))
                 (fn   (expand-file-name (format "gpt-%s-%s.org" ts slug) dir)))
...
#+begin_src elisp
(setup gptel-agent
  ;; (keymap-global-set "C-M-S-p" #'gptel-simple-agent)
