...
(setup mini-echo
  ...
  (setq mini-echo-persistent-rule
        '(:long
          ("major-mode" "vcs" "flymake" "process"
           "selection-info" "narrow"  "profiler" "repeat" "eglot" "buffer-size" "buffer-position" "shrink-path"  "meow" "macro" "org-clock")
          :short
  ...
  (mini-echo-define-segment "org-clock"
   "Return org-clock."
   :fetch (when (featurep 'org-clock) (when (org-clocking-p) (org-clock-get-clock-string)))
   )
  (mini-echo-mode)
  )
#+end_src
...
(setup autoinsert
  ...
  (define-auto-insert "\\.excalidraw\\'"
    (lambda ()
      (insert org-excalidraw--default-base)))
  )
#+end_src
** clue
...
(setup org-fc
  ...
  (:when-loaded
    (set-face-attribute 'org-fc-type-cloze-hole-face nil :box t :foreground "pale violet red")
    ...
    (add-to-list 'org-fc-custom-contexts
                 '(not-low . (:filter (not (tag "low")))))
    (add-to-list 'org-fc-custom-contexts
                 '(only-review . (:filter (type cloze) :order shuffled)))
    )
  ;; +-----------------+
  ;; | load and keymap |
  ;; +-----------------+
  ...
  (defface my-org-fc-cloze-face
    '((t (:weight bold :box t :foreground "pale violet red")))
    "A custom face with bold weight and a box.")

  ;; font lock for cloze
  (defun org-fc-cloze-in-org-font-lock ()
    (font-lock-add-keywords
     nil
     '(("{{\\([^}]+\\)}@[0-9]+}"
        (1 'my-org-fc-cloze-face t)
        ))))
  (add-hook 'org-mode-hook #'org-fc-cloze-in-org-font-lock)

  (defun org-fc-clarify-continue-reading ()
    "By determining whether all the cards in the file containing the article have been reviewed, decide whether this article can continue to be read."
    (interactive)
    (dolist (card (org-fc-index '(:filter (and (tag "suspended") (and (tag "article"))))))
      (let ((file_path (oref (oref card file) path))
        ...
        (dolist (file-card (org-fc-index `(:paths ,file_path :filter (not (tag "suspended")))))
          (if (< (plist-get (oref (car (oref file-card positions)) data) :interval)  1)
...
(setup atomic-chrome
  ...
  (setq atomic-chrome-create-file-strategy '(("/tmp/" :extension ("cpp" "c" "py" )))
