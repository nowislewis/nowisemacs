...
#+begin_src elisp
  (setup lsp-bridge
    (:option lsp-bridge-org-babel-lang-list '("emacs-lisp"
             ...
             ;; acm-backend-search-sdcv-words-dictionary (file-truename "~/Downloads/stardicts/stardict-langdao-ec-gb-2.4.2/langdao-ec-gb")
             ))
...
#+begin_src elisp
  (setup ebib
    (:option ebib-preload-bib-files org-cite-global-bibliography
             ...
             ebib-bib-search-dirs '("~/Documents/emacs/03-bibliography/")
             ebib-default-directory 'first-bib-dir
             ebib-file-associations '(("pdf" . "xpdf")
                                      ("ps" . "gv")
                                      ("epub"))
             ebib-index-window-size 25
             ebib-reading-list-file "~/Documents/emacs/01-orgmode/xnotes/20230403T125743--ebib-reading-lists.org"
             )
    (:when-loaded
...
#+begin_src elisp
  (defvar nowisemacs-index-org (concat nowisemacs-gtd-dir "/20230411T002918--index.org"))
  (defvar nowisemacs-gtd-org (concat nowisemacs-gtd-dir "/20230411T002424--action.org"))
  (defvar nowisemacs-someday-org (concat nowisemacs-gtd-dir "/20230310T005150--someday.org"))
  (setup org
    (:option org-agenda-files (list nowisemacs-index-org
                                    nowisemacs-gtd-org
                                    ;; (concat nowisemacs-gtd-dir "/20230411T002918--index.org_archive")
                                    ;; (concat nowisemacs-gtd-dir "/20230411T002424--action.org_archive")
                                    )
...
#+begin_src elisp
  ...
  ;; TODO use let to add all files under gtd in agenda-files for full-search
#+end_src
** org-ql
#+begin_src elisp
  (setup ts)
...
#+begin_src elisp
  ...
  (setup dictionary-overlay
    (:option dictionary-overlay-sdcv-dictionary-path "~/Documents/emacs/02-binary-files/03-stardicts/")
...
#+begin_src elisp
  (setup sdcv
    (:option sdcv-dictionary-data-dir (file-truename "~/Documents/emacs/02-binary-files/03-stardicts/")
...
*** backup scripts
#+begin_src elisp
  (defun lewis-backup-to-nas-rsync()
    (interactive)
    (async-shell-command "rsync -avbizh --delete --backup-dir=../back_restore/ ~/Documents/emacs/02-binary-files/02-searchable/ lewis-nas@192.168.31.108:/share/my_webdav/rsync/02-searchable/")
    )
  (defun lewis-backup-to-pc-rsync()
    (interactive)
    (async-shell-command "rsync -avbizh --delete --backup-dir=../back_restore/ lewis-nas@192.168.31.108:/share/my_webdav/rsync/02-searchable/  ~/Documents/emacs/02-binary-files/02-searchable/")
    )

  (defun lewis-backup-to-nas-restic()
    (interactive)
    (async-shell-command "~/Documents/my-bins/restic -r sftp:lewis-nas@192.168.31.108:/share/my_webdav/restic backup ~/Documents/emacs/02-binary-files/")
    )
#+end_src
*** startup scripts
#+begin_src elisp
  (defun lewis-start-zotero-server()
    "Starts the Zotero server by opening a new eshell, changing to the translation-server directory, and running 'npm start'."
    (interactive)
    (let ((default-directory "~/Downloads/translation-server"))
      (eshell)
      (rename-buffer "zotero-server")
