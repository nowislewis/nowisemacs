...
(setup ghostel
  ...
  (autoload 'nowis-ghostel-toggle-window "nowis-ghostel" nil t)
  (keymap-global-set "M-`" #'nowis-ghostel-toggle-window)
  (:when-loaded
  ...
  (:when-loaded
    (setq-default gptel-backend
                  (gptel-make-anthropic "momenta"
                    ...
                    :key 'gptel-api-key
                    :stream t
                    :models '(claude-opus-5 claude-sonnet-5 gpt-5.6-sol gpt-5.6-terra deepseek-v4-flash)))
    (gptel-make-openai "momenta2"
      :host "llm-gateway.momenta.works"
      :key 'gptel-api-key
      :stream t
    ...
    (dolist (model '(claude-opus-5 claude-sonnet-5 claude-fable-5
...
** wslg
#+begin_src elisp
;; WSLg's Wayland clipboard exposes Windows text both as UTF-8 and the
;; legacy STRING target.  Emacs 32's PGTK selection backend currently falls
;; through to STRING, which decodes Chinese text as Latin-1.  Read the UTF-8
;; MIME target through wl-paste instead, while leaving normal Linux desktops
;; on Emacs's native clipboard path.
(when (and (eq system-type 'gnu/linux)
           (getenv "WSL_DISTRO_NAME")
           (eq window-system 'pgtk)
           (executable-find "wl-paste"))
  (defun nowis/wslg-clipboard-paste ()
    "Return newly copied WSLg UTF-8 text for `current-kill'.
Return nil when the system clipboard still contains Emacs's latest kill, so
that `current-kill' retains the local kill-ring entry instead of re-adding it."
    (let ((clipboard
           (with-temp-buffer
             (let ((coding-system-for-read 'utf-8-unix))
               (when (zerop (call-process "wl-paste" nil t nil
                 ...
                 (buffer-string))))))
      (unless (or (string-empty-p clipboard)
                  (equal clipboard (substring-no-properties (car kill-ring))))
        clipboard)))
  (setq interprogram-paste-function #'nowis/wslg-clipboard-paste))
#+end_src
