...
(defun eat-toggle-window()
  ...
  (interactive)
  (let ((current-eat-window
         (seq-find (lambda (win)
                     (with-current-buffer (window-buffer win)
                       (eq major-mode 'eat-mode)))
                   (window-list))))
    (if current-eat-window
        (delete-window current-eat-window)
      (let ((eat-buffers (eat-cycle--get-buffers)))
        (if eat-buffers
            (progn
              (pop-to-buffer (car eat-buffers))
              (eat-cycle))
          (progn
            (unless (featurep 'eat)
              (require 'eat)
...
(defun eat-cycle--get-buffers ()
  (seq-filter (lambda (b) (eq 'eat-mode (buffer-local-value 'major-mode b)))
              (buffer-list)))

(defun eat-cycle--assign-slots (buffers)
  "Assign persistent slot numbers to buffers."
  ;; Clean up dead
  (let ((live-names (mapcar #'buffer-name buffers)))
  ...
  (let ((next 0))
    (cl-loop for buf in buffers
             ...
             for slot = (or (gethash name eat-cycle--registry)
                            (prog1 next
                              (puthash name next eat-cycle--registry)
                              (setq next (1+ next))))
             when (< slot (length eat-cycle-keys))
             collect (cons slot buf))))

;;;###autoload
(defun eat-cycle ()
  "Cycle through eat buffers with numbered quick access."
  (interactive)
  (let* ((buffers (eat-cycle--get-buffers))
         (slots (sort (eat-cycle--assign-slots buffers)
                      (lambda (a b) (< (car a) (car b))))))
    (pcase (length buffers)
      (0 (message "No eat buffers"))
      (1 (switch-to-buffer (car buffers)))
      (_ (let ((current (current-buffer)))
                    ...
                    (mapconcat
                     (lambda (s)
                       (let* ((idx (1+ (car s)))
                              ...
                              (is-current (eq buf current)))
                         (if is-current
                             (propertize (format "[%d:%s]" idx name)
           ...
           (set-transient-map
            (let ((map (make-sparse-keymap)))
              (dolist (s slots)
                (define-key map (kbd (nth (car s) eat-cycle-keys))
                            `(lambda () (interactive)
                               (switch-to-buffer ,(cdr s))
                               (eat-cycle))))
              map)))))))
  )
#+end_src
