...
** other check
#+begin_src elisp
(defvar nowis/is-batch-open
  (and command-line-args-left        ; 有文件参数
       (not (daemonp)))              ; 不是 daemon 模式
  "Non-nil when started as `emacs FILE'.")
#+end_src
* Keyboard Bindings
...
*** pulsar
#+begin_src elisp
;; Use built-in pulse.el instead of third-party pulsar
(require 'pulse)
...
(setq agent-shell-thought-process-icon "󰟶")
#+end_src
* webjump
#+begin_src elisp
(setup webjump
