...
- [[#search][Search]]
  ...
  - [[#rgel][rg.el]]
- [[#补全][补全]]
  - [[#default-completion-comfigurations][default completion comfigurations]]
...
- [[#org-mode][Org Mode]]
  ...
  - [[#org-srs][org-srs]]
  - [[#org-inc][org-inc]]
  - [[#inline-anki][inline-anki]]
  - [[#doc-capture][doc-capture]]
  - [[#insert-from-clipboard][insert from clipboard]]
  - [[#org-sliced-images][org-sliced-images]]
  - [[#org-graphviz-mindmap][org-graphviz-mindmap]]
  - [[#org-ladder][org-ladder]]
- [[#阅读][阅读]]
  - [[#pdf-tools][pdf-tools]]
  - [[#pdf-view-pagemark][pdf-view-pagemark]]
  - [[#pdf-to-org-mode][pdf-to-org-mode]]
  - [[#reader][reader]]
...
2. =01-orgmode= stores .org files, where =xnote= is the default directory of =Denote= .
...
(transient-define-prefix notes-leader-map()
  ...
  [["Roam"
    ...
    ("r w" "week-target" nowis/denote-journal-this-week-major-tasks)
   ...
   ["graph"
    ("g" "org-graphviz-mindmap" org-graphviz-mindmap-create)
    ]
   ["ladder"
...
(transient-define-prefix apps-leader-map()
  ...
  [["agenda"
    ...
    ("p" "planning today" nowis/goto-or-capture-daily-plan)
    ("s" "summary today" nowis/goto-or-capture-daily-summary)
    ("o" "occur" (lambda ()
                   (interactive)
                   (org-occur "^[ \t]*\\*+ +\\(TODO\\|NEXT\\)\\>")))
    ]
  ...
  (defun eat-toggle-window()
    ...
    (let ((eat-win (seq-find (lambda (w)
      ...
      (if eat-win
        ...
        (if-let ((buf (car (eat-cycle--get-buffers))))
            (progn (pop-to-buffer buf) (eat-cycle))
          (unless (featurep 'eat) (require 'eat) (eat-reload))
          (eat)
          (eat-cycle)))))

  (defvar eat-cycle-keys '("M-1" "M-2" "M-3" "M-4" "M-5" "M-6" "M-7" "M-8" "M-9"))
  (defvar eat-cycle--slots (make-hash-table :test 'equal)
    "Maps buffer-name -> slot-number for persistent assignment.")

  (defun eat-cycle--get-buffers ()
    "Return live eat buffers sorted by assigned slot."
    (let ((bufs (seq-filter (lambda (b)
      ...
      (maphash (lambda (name _)
                 (unless (get-buffer name)
                   (remhash name eat-cycle--slots)))
               eat-cycle--slots)
      ;; Sort by slot number
      (sort bufs (lambda (a b)
                   (< (or (gethash (buffer-name a) eat-cycle--slots) 999)
                      (or (gethash (buffer-name b) eat-cycle--slots) 999))))))

  (defun eat-cycle--ensure-slot (buffer)
    "Ensure buffer has a slot assigned, return (slot . buffer)."
    (let* ((name (buffer-name buffer))
           (slot (gethash name eat-cycle--slots)))
      (unless slot
        ;; Find first free slot
        (let ((used (hash-table-values eat-cycle--slots)))
          (setq slot (cl-loop for i from 0 below (length eat-cycle-keys)
                              unless (memq i used) return i)))
        (when slot (puthash name slot eat-cycle--slots)))
      (when slot (cons slot buffer))))

;;;###autoload
  (defun eat-cycle ()
    "Cycle through eat buffers with persistent slot numbers."
    (interactive)
    (let* ((buffers (eat-cycle--get-buffers))
           (slots (delq nil (mapcar #'eat-cycle--ensure-slot buffers)))
           (current (current-buffer)))
      (cond
       ((null buffers) (message "No eat buffers"))
       ((= 1 (length buffers)) (switch-to-buffer (car buffers)))
       (t
        (unless (memq current (mapcar #'cdr slots))
                 ...
                 (mapconcat (lambda (s)
                              (let ((idx (1+ (car s)))
                                ...
                                (if (eq buf (current-buffer))
                                    (propertize (format "[%d:%s]" idx (buffer-name buf))
                                                'face '(:foreground "green" :weight bold))
                                  (format " %d:%s " idx (buffer-name buf)))))
                            slots " "))
        ;; Setup transient map
        (set-transient-map
         (let ((map (make-sparse-keymap)))
           (dolist (s slots)
             (let* ((slot-idx (car s))
                    (target-buf (cdr s)))  ; 使用 let* 正确捕获变量
               (define-key map (kbd (nth slot-idx eat-cycle-keys))
                           `(lambda ()
                              (interactive)
...
** vterm
#+begin_src elisp :tangle no
  (defun meomacs-vterm-meow-setup()
    (add-hook 'meow-normal-mode-hook 'vterm-copy-mode nil t)
    (add-hook 'meow-insert-mode-hook (lambda () (vterm-copy-mode -1)) nil t))

  (with-eval-after-load "meow"
    (with-eval-after-load "vterm"
      (keymap-set vterm-mode-map "C-S-v" 'vterm-yank)
      (define-key vterm-mode-map (kbd "C-\\") nil)
      (add-hook 'vterm-mode-hook 'meomacs-vterm-meow-setup)))

  (setup vterm
    (keymap-global-set "M-`" #'vterm-toggle-window)
    ...
    (defun vterm-toggle-window()
      "Toggle vterm buffer window."
      (interactive)
      (let ((vterm-win (seq-find (lambda (w)
                                   (eq 'vterm-mode (buffer-local-value 'major-mode (window-buffer w))))
                                 (window-list))))
        (if vterm-win
            (delete-window vterm-win)
          (if-let ((buf (car (vterm-cycle--get-buffers))))
              (progn (pop-to-buffer buf) (vterm-cycle))
            (unless (featurep 'vterm) (require 'vterm))
            (vterm)
            (vterm-cycle)))))

    (defvar vterm-cycle-keys '("M-1" "M-2" "M-3" "M-4" "M-5" "M-6" "M-7" "M-8" "M-9"))
    (defvar vterm-cycle--slots (make-hash-table :test 'equal)
      "Maps buffer-name -> slot-number for persistent assignment.")

    (defun vterm-cycle--get-buffers ()
      "Return live vterm buffers sorted by assigned slot."
      (let ((bufs (seq-filter (lambda (b)
                                (and (buffer-live-p b)
        ...
        (maphash (lambda (name _)
                   (unless (get-buffer name)
                     (remhash name vterm-cycle--slots)))
                 vterm-cycle--slots)
        ;; Sort by slot number
        (sort bufs (lambda (a b)
                     (< (or (gethash (buffer-name a) vterm-cycle--slots) 999)
                        (or (gethash (buffer-name b) vterm-cycle--slots) 999))))))

    (defun vterm-cycle--ensure-slot (buffer)
      "Ensure buffer has a slot assigned, return (slot . buffer)."
      (let* ((name (buffer-name buffer))
             (slot (gethash name vterm-cycle--slots)))
        (unless slot
          ;; Find first free slot
          (let ((used (hash-table-values vterm-cycle--slots)))
            (setq slot (cl-loop for i from 0 below (length vterm-cycle-keys)
                                unless (memq i used) return i)))
          (when slot (puthash name slot vterm-cycle--slots)))
        (when slot (cons slot buffer))))

  ;;;###autoload
    (defun vterm-cycle ()
      "Cycle through vterm buffers with persistent slot numbers."
      (interactive)
      (let* ((buffers (vterm-cycle--get-buffers))
             (slots (delq nil (mapcar #'vterm-cycle--ensure-slot buffers)))
             (current (current-buffer)))
        (cond
         ((null buffers) (message "No vterm buffers"))
         ((= 1 (length buffers)) (switch-to-buffer (car buffers)))
         (t
          (unless (memq current (mapcar #'cdr slots))
            (switch-to-buffer (cdar slots)))
          ...
          (set-transient-map
           (let ((map (make-sparse-keymap)))
             (dolist (s slots)
               (let* ((slot-idx (car s))
                      (target-buf (cdr s)))
                 (define-key map (kbd (nth slot-idx vterm-cycle-keys))
                             `(lambda ()
                                (interactive)
                                (switch-to-buffer ,target-buf)
                                (vterm-cycle)))))
...
(defun nowis/org-toggle-sidebar()
  ...
  (let* ((sidebar-buffer-name (concat "*Outline: " (buffer-name) "*"))
         (window-width (/ (frame-width) 4))) ; Set width to one-third of the screen width
...
(setup denote-journal
  ...
  (defun nowis/denote-journal-this-week-major-tasks ()
    "Open this week's Saturday journal and jump to or insert '本周重点任务' headline."
    (interactive)
    (let ((date (nowis/current-week-saturday)))
...
(setup org
  (:when-loaded
    (setq org-capture-templates
          `(
            ("p" "Plan" entry (file+olp+datetree "gtd/inbox.org")
             ,(concat "* Daily Plan\n"
                      ":PROPERTIES:\n"
                      ":ENTRY_TYPE: plan\n"
                      ":CREATED: %U\n"
                      ":END:\n%?"))
            ("s" "Summary" entry (file+olp+datetree "gtd/inbox.org")
             ,(concat "* Daily Summary\n"
                      ":PROPERTIES:\n"
                      ":ENTRY_TYPE: summary\n"
                      ":CREATED: %U\n"
                      ":END:\n%?"))
            ...
            ("d" "Done without clock" entry (file+olp+datetree "gtd/inbox.org")
             ,(concat "* DONE %?\n"
                      "CLOSED: %U"))
            ("t" "org-inc topic" entry (file ,(concat org-inc-directory "20251201T112101--incubate.org"))
             ,(concat "* %?\n" ":PROPERTIES:\n:CREATED: %U\n:END:\n")
...
(defun nowis/goto-or-capture-daily-entry (entry-type capture-key)
  "通用函数: 跳转或创建每日条目"
  (let* ((file (expand-file-name "gtd/inbox.org" org-directory))
         (today-regexp (format-time-string "\\*\\{3,\\} %Y-%m-%d %a"))
         found-pos)

    (with-current-buffer (find-file-noselect file)
      (save-excursion
        (goto-char (point-min))
        (when (re-search-forward today-regexp nil t)
          (let ((tree-end (save-excursion (org-end-of-subtree t t))))
            (while (and (not found-pos)
                       (re-search-forward "^\\*\\{4,\\}" tree-end t))
              (when (string= (org-entry-get nil "ENTRY_TYPE") entry-type)
...
(defun nowis/goto-or-capture-daily-plan ()
  "智能跳转或创建今日计划"
  (interactive)
  (nowis/goto-or-capture-daily-entry "plan" "p"))

(defun nowis/goto-or-capture-daily-summary ()
  "智能跳转或创建今日总结"
  (interactive)
  (nowis/goto-or-capture-daily-entry "summary" "s"))
#+end_src
*** refile
#+begin_src elisp
(setup org
...
;; custom skip functins
(defun my-org-agenda-skip-blocked ()
  (when (org-entry-blocked-p)
    (or (outline-next-heading) (point-max))))

(defun my-org-agenda-skip-unblocked ()
  (unless (org-entry-blocked-p)
    (or (outline-next-heading) (point-max))))

;; 定义统一的 prefix format
(defvar nowis/agenda-prefix-format-basic "%-20:(org-get-parent-heading): [%-4e] "
  "Basic agenda prefix format.")

(defvar nowis/agenda-prefix-format-enhanced
  "%-20:(org-get-parent-heading): [%-4e] [%3(or (org-entry-get nil \"value\") \"\")] [%4(if (and (org-entry-get nil \"value\") (org-entry-get nil \"Effort\")) (format \"%.2f\" (/ (string-to-number (org-entry-get nil \"value\")) (org-duration-to-minutes (org-entry-get nil \"Effort\")))) \"\")] "
  "Enhanced agenda prefix format with value and effort/value ratio.")
         ...
         '(
           (tags-todo ,(concat "+week" tag)
                      ((org-agenda-overriding-header
                       ...
                       (org-agenda-prefix-format nowis/agenda-prefix-format-enhanced)))
           (agenda ""
           ...
           (tags-todo "inbox-TODO=\"NEXT\"-PRIORITY=\"E\""
                      ((org-agenda-overriding-header (nowis/align-to-window-center "Inbox"))
                       (org-agenda-skip-function #'my-org-agenda-skip-blocked)
                       (org-agenda-prefix-format nowis/agenda-prefix-format-enhanced)
                       (org-agenda-sorting-strategy '(todo-state-up priority-down timestamp-up))))
           ;; (tags-todo ,(concat "+action" tag) ((org-agenda-overriding-header (nowis/align-to-window-center "Soft Time"))
           ;;                                     (org-agenda-prefix-format "%-20:(org-get-parent-heading): [%-4e] ")
           ;;                                     (org-agenda-sorting-strategy '(todo-state-up priority-down timestamp-up))
           ;;                                     ))
           (tags-todo "inbox-TODO=\"NEXT\"+PRIORITY=\"E\""
                      ((org-agenda-overriding-header (nowis/align-to-window-center "Low Priority"))
                       (org-agenda-skip-function #'my-org-agenda-skip-blocked)
                       (org-agenda-prefix-format nowis/agenda-prefix-format-enhanced)
                       (org-agenda-sorting-strategy '(todo-state-up priority-down timestamp-up))))
           (tags-todo "inbox-TODO=\"NEXT\""
                      ((org-agenda-overriding-header (nowis/align-to-window-center "Blocked"))
                       (org-agenda-skip-function #'my-org-agenda-skip-unblocked)
                       (org-agenda-prefix-format nowis/agenda-prefix-format-basic)
                       (org-agenda-sorting-strategy '(todo-state-up priority-down timestamp-up))))
...
(setup asyncloop)
(setup inline-anki
(defvar-local org-inline-anki-overlays nil)

(defun org-inline-anki-overlay-update ()
  "Update all inline anki overlays."
  (mapc #'delete-overlay org-inline-anki-overlays)
  (setq org-inline-anki-overlays nil)
  (when org-inline-anki-overlay-mode
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward "@\\^{\\([0-9]+\\)}" nil t)
        (let ((ov (make-overlay (match-beginning 0) (match-end 0))))
          (overlay-put ov 'display
                       (propertize " 󰯂" 'face '(:foreground "#98C379" :weight bold)))
          (push ov org-inline-anki-overlays))))))

(define-minor-mode org-inline-anki-overlay-mode
  "Toggle @^{timestamp} beautification for inline anki."
  :lighter " 󰯂"
  (org-inline-anki-overlay-update)
  (if org-inline-anki-overlay-mode
      (add-hook 'after-save-hook #'org-inline-anki-overlay-update nil t)
    (remove-hook 'after-save-hook #'org-inline-anki-overlay-update t)))

      ;; (add-hook 'org-mode-hook #'org-inline-anki-overlay-mode)
...
#+begin_src elisp
(setup doc-capture
  (:autoload doc-capture-process)
  (setq doc-capture-org-directory (concat org-inc-directory "doc-capture/"))
  (:when-loaded
...
#+end_src
** org-graphviz-mindmap
#+begin_src elisp
(setup org-graphviz-mindmap)
#+end_src
** org-ladder
#+begin_src elisp
(setup org-ladder
...
** pdf-to-org-mode
#+begin_src elisp
(defun nowis/pdf-to-org ()
  "convert PDF to org-mode use mutool and pandoc"
  (interactive)
  (let ((pdf (buffer-file-name))
        (org (concat (file-name-sans-extension (buffer-file-name)) ".org")))
    (shell-command (format "mutool draw -F xhtml %s | pandoc -f html -t org -o %s" pdf org))
    (find-file org)))
#+end_src
** reader
#+begin_src elisp
  ...
  (:when-loaded
    (setq-default gptel-backend
                  (gptel-make-openai "momenta"
                    ...
                    :key 'gptel-api-key
                    :stream t
                    :models '(claude-sonnet-4.5 claude-haiku-4.5 minimax-m2.1 gpt-5.2 gemini-3-pro qwen3-coder-plus deepseek-v3.2)))
    (gptel-make-openai "chatanywhere"
      :host "api.chatanywhere.tech"
      ;; :header (lambda () `(("Authorization" . ,(concat "Bearer " (gptel--get-api-key)))))
      :key 'gptel-api-key
      :stream t
      :models '(gpt-5 deepseek-v3.1-250821 claude-haiku-4-5-20251001))
...
(setup gptel-aibo
  (:when-loaded
    (define-key gptel-aibo-mode-map
      ...
      (keymap-global-set "M-c" #'gptel-aibo-summon)
    ))
#+end_src
