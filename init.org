...
(setup eglot
  ...
  (add-hook 'prog-mode-hook (lambda ()
                              (unless (derived-mode-p 'emacs-lisp-mode 'lisp-mode 'makefile-mode 'snippet-mode)
                                (eglot-ensure))))
  ;; (add-hook 'org-mode-hook (lambda() (eglot-ensure)))
...
(setup org
  (:when-loaded
    (defun my/current-week-saturday ()
      "Return the date string for this week's Saturday in YYYY-MM-DD format."
      (let* ((now (current-time))
             (dow (string-to-number (format-time-string "%u" now))) ;; 1=Mon ... 7=Sun
             (offset (- 6 dow)) ;; 6为Sat, 1为Mon
             (saturday (time-add now (days-to-time offset))))
        saturday))
    ...
    (defun org-agenda-update-agenda-files ()
      ...
      (let* ((gtd-dir (concat org-directory "/gtd"))
             ...
             (journal-files
              (progn
                (unless (featurep 'denote-journal)
                  (require 'denote-journal))
                (let ((dates (list (current-time) (my/current-week-saturday)))) ;; add today and current saturday
                  (apply #'append
                         (delq nil
                               (mapcar #'denote-journal--entry-today dates))))))) ;;remove nil
        (setq org-agenda-files (append gtd-files journal-files)))
  ...
  (:when-loaded
    (setq org-capture-templates
          `(("i" "Inbox" entry  (file+datetree "gtd/inbox.org")
             ,(concat "* TODO %?\n"
                      "/Entered on/ %U"))
            ("d" "Done" entry  (file+datetree "gtd/inbox.org")
             ,(concat "* DONE %?\n"
                      "CLOSED: %U"))
...
;; give refile a priority
;; (defun my/org-set-priority-after-refile (&rest _)
;;   "Prompt to set Org priority after refiling."
;;   (org-priority))

;; (advice-add 'org-refile :before #'my/org-set-priority-after-refile)
...
(defmacro nowis-create-agenda-command (key description tag)
  `(list ,key ,description
         '(
           ...
           ;; (tags-todo ,(concat "+journal-week" tag)
           ;;            ((org-agenda-overriding-header (nowis-align-to-window-center "Today's Target"))
      ...
      (dolist (file files)
        (with-current-buffer (find-file file)
          ...
          (when (derived-mode-p 'org-mode)
            (when (local-variable-p 'org-srs-review-item)
              (org-srs-review-end))
...
#+begin_src elisp
(setup gptel-prompts
  (setq gptel-prompts-directory (concat nowis-doc-emacs-dir "99-gptel-prompts"))
...
(setup whisper
  ...
  (:when-loaded
    (if (string= "Arch" (getenv "WSL_DISTRO_NAME"))
        (progn
          (setq sense_bin "~/.emacs.d/.cache/SenseVoice.cpp/build/bin/sense-voice-main")
          (setq sense_model (expand-file-name "~/.emacs.d/.cache/SenseVoice.cpp/sense-voice-small-fp32.gguf"))
          (defun whisper--sensevoice-command (input-file)
            `(,sense_bin
              "-m" ,sense_model
              "-t" "12"
              "-l" "zh"
              ...
              ,input-file))
          (advice-add 'whisper-command :override #'whisper--sensevoice-command)
          ;; scene voice can only use AC one
