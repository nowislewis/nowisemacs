...
(setup treesit
  (setq treesit-font-lock-level 4
        ...
        treesit-extra-load-path (list (concat user-emacs-directory "var/treesit/")))
...
(setup org
  ...
  (defun org-agenda-update-agenda-files ()
    ...
    (let ((gtd-files (nowis/gtd-files)))
      (setq org-agenda-files (cl-remove-if (lambda (f) (string-match-p "incubate" f))
                                           gtd-files)
            org-refile-files (cl-remove-if (lambda (f) (string-match-p "inbox\\|agenda\\|reading" f))
                                           gtd-files))))
