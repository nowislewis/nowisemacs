...
(add-hook 'delete-frame-functions
          (lambda (frame)
            (when (display-graphic-p frame)
              (ignore-errors (desktop-save desktop-dirname t)))))
;; attach: restore the full layout onto the new graphical frame.
;; (add-hook 'server-after-make-frame-hook
;;           (lambda ()
;;             (when (display-graphic-p)
;;               (ignore-errors (desktop-read desktop-dirname)))))
)
#+end_src
** project
#+begin_src elisp
