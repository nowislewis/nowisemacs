...
(transient-define-prefix apps-leader-map()
  ...
  [["agenda"
   ...
   ["others"
    ...
    ("w" "whisper" whisper-api-record-dwim)
    ]
   ])
#+end_src
*** ui
...
(defun ace-window-one-command ()
  ...
  ;; (keymap-global-set "M-O" #'ace-window-one-command)
...
With prefix argument CHOOSE-BUFFER, prompt for buffer name."
  (interactive "P")
  (let ((eat-win (seq-find (lambda (w)
    ...
    (if eat-win
      ...
      (if choose-buffer
          ;; prompt to choosing buffer
          (let* ((existing-bufs (mapcar #'buffer-name (eat--get-buffers)))
                 (buf-name (completing-read "Choose eat buffer: "
                                            existing-bufs
                                            nil nil))
            ...
            (if buf
              ...
              (let ((eat-buffer-name buf-name))
                (eat))))
        ;; without prefix
        (if-let ((buf (car (eat--get-buffers))))
            (pop-to-buffer buf)
          (unless (featurep 'eat) (require 'eat) (eat-reload))
          (eat))))))
...
#+begin_src elisp
(setup whisper-api
  (:autoload whisper-api-record-dwim)
  (setq whisper-api-base-url "https://api.chatanywhere.tech/v1/audio/transcriptions"
        whisper-api-model "gpt-4o-mini-transcribe"
        whisper-api-max-recording-seconds 600
        )
  (if (eq system-type 'darwin)
      (setq whisper-api-ffmpeg-command "ffmpeg -y -t %d -f avfoundation -i \":1\" -ar 16000 %s"))
  )
#+end_src
*** eca
#+begin_src elisp
