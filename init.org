...
(defun ghostel-toggle-window (&optional choose-buffer)
  "Toggle ghostel window. With C-u, prompt to select or create a buffer."
  (interactive "P")
  (if-let* ((win (seq-find (lambda (w)
    ...
    (if choose-buffer
        (let* ((bufs (seq-filter (lambda (b)
          ...
          (if buf
              (pop-to-buffer buf)
            (let ((ghostel-buffer-name (format "*%s*" (string-trim choice "*" "*"))))
              (ghostel))))
  ...
  (transient-define-prefix org-sm-transient-map()
    ...
    [
     ...
     ["gpt"
      ...
      ("A" "PSA analyze" org-sm-gptel-analyze)
      ]
     ]
    )
  )
