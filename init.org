...
#+begin_src elisp :tangle no
  (let ((emacs-font-size 14)
    ...
    (when (display-grayscale-p)
      ...
      (set-fontset-font (frame-parameter nil 'font) 'unicode "等距更纱黑体 SC")
      ))
#+end_src
...
#+begin_src elisp
  (if (display-graphic-p)
      (setup (:pkg company-quickhelp)
        (:hook-into global-company-mode)
        (:option company-quickhelp-delay 0.2))
    (setup (:pkg company-quickhelp-terminal)
      (:hook-into global-company-mode)
      (:option company-quickhelp-delay 0.2))
    )
...
#+begin_src elisp
  (setup (:pkg lsp-mode)
    ...
    (:option lsp-clients-svlangserver-launchConfiguration "/tools/verilator -sv --lint-only -Wall"
