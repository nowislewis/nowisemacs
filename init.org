...
#+begin_src elisp
  (transient-define-prefix leader-map-for-notes()
    "Define notes leader-key maps"
    [["find notes/papers"
      ("f" "find notes" project-find-denote-files)
      ("p" "find papers" project-find-paper-files)]
     ["New notes"
      ("n" "denote" denote)
      ("d" "date" denote-date)
      ("t" "type" denote-type)
...
#+begin_src elisp
  ...
  (defun project-find-denote-files (&optional include-all)
    (interactive "P")
    (let* ((pr (project-current t))
           (root (concat denote-directory "xnotes"))
           (dirs (list root)))
  ...
  (defun project-find-paper-files (&optional include-all)
    (interactive "P")
    (let* ((pr (project-current t))
           (root (concat denote-directory "papers"))
           (dirs (list root)))
      (project-find-file-in
       (or (thing-at-point 'filename)
           (and buffer-file-name (file-relative-name buffer-file-name root)))
