...
(add-hook 'org-agenda-finalize-hook #'nowis/org-setup-prettify)
#+end_src
*** org-appear
#+begin_src elisp
(setup org-appear
...
(setup org
  (:when-loaded
    (setq org-capture-templates
          `(("i" "Inbox" entry (file+headline "gtd/action.org" "Inbox")
             ,(concat "* TODO [#C] %?\n"
                      ":PROPERTIES:\n:CREATED: %U\n:END:\n"))
            ("d" "Done" entry (file+headline "gtd/action.org" "Inbox")
...
MARKER 空=用当前 agenda 行；非空=跨 buffer 取值（排序用）。"
  (if marker
...
(defun nowis/recent-done-files ()
  ...
  (cl-remove-if-not
   ...
   (append (nowis/gtd-files)
           (list (concat (nowis/gtd-dir) "/gtd_archive")))))
