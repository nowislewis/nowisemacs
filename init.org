...
(defun my/open-with-zathura (file)
  "Open FILE with zathura."
  (start-process "zathura" nil "zathura" file))

(with-eval-after-load 'embark
  (define-key embark-file-map (kbd "z") #'my/open-with-zathura))
#+end_src
** yasnippet
#+begin_src elisp
(setup yasnippet
...
(setup org
  (:when-loaded
    (defcustom my/org-yank-image-filename-function
      (lambda () (format-time-string "screenshot_%Y%m%d_%H%M%S.png"))
      "A function to generate screenshot name"
      :type 'function
      :group 'org)

    (defun my/org-yank-image-from-clipboard-wsl ()
      "paste from wsl"
      ;; (interactive)
      (require 'org-attach)
      (let* ((attach-dir (org-attach-dir t))
             (filename (funcall my/org-yank-image-filename-function))
    ...
    (defun my/org-yank-image-from-clipboard-native ()
      "use the default yank-media"
      (yank-media))

    (defun my/org-yank-image-from-clipboard ()
      "paste pictures from powershell when using wsl, others use yank-media"
      (interactive)
      (if (string= "Arch" (getenv "WSL_DISTRO_NAME"))
          (my/org-yank-image-from-clipboard-wsl)
        (my/org-yank-image-from-clipboard-native)))
    (:with-map org-mode-map
      (:bind "M-<f1>" my/org-yank-image-from-clipboard)
      )
    ))
#+end_src
** org-sliced-images
