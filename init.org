...
(require 'batch-build-pkgs)
(batch-build-pkgs-initialize)
#+end_src
** drop out
...
(setup org-srs
  ...
  (defun org-cloze-fontify (limit)
    ...
    (while (re-search-forward "{{\\([0-9a-z]+\\)}{\\([^}]+\\)}}" limit t)
      (add-text-properties (match-beginning 0) (match-beginning 2)
                           '(invisible org-cloze font-lock-multiline t))
      (put-text-property (match-beginning 0) (1+ (match-beginning 0))
                         'display "{")
      (add-text-properties (match-beginning 2) (match-end 2)
                           '(face my-org-emphasis-bold font-lock-multiline t))
      ...
      (add-text-properties (1- (match-end 0)) (match-end 0)
                           '(rear-nonsticky (invisible display)))))
  ...
  (define-minor-mode org-cloze-overlay-mode
    ...
    (if org-cloze-overlay-mode
        (progn
          (add-to-invisibility-spec 'org-cloze)
          (font-lock-add-keywords nil '((org-cloze-fontify)))
          (font-lock-flush))
      (progn
        (remove-from-invisibility-spec 'org-cloze)
        (font-lock-remove-keywords nil '((org-cloze-fontify)))
        (font-lock-flush))))
  )
#+end_src
** org-inc
