...
#+begin_src elisp
(setopt completions-detailed t  ;;useful in emacs 28
      completions-group t
      completions-sort 'historical
      completion-auto-help 'lazy ;; turn off *Completions* buffer
)
#+end_src
...
(define-keymap :keymap icomplete-minibuffer-map
  ...
  "<remap> <backward-delete-char>"         #'icomplete-fido-backward-updir
  "<remap> <delete-backward-char>"         #'icomplete-fido-backward-updir
  "RET"                                    #'icomplete-force-complete-and-exit
  "C-j"                                    #'minibuffer-complete-and-exit)

;; could highlight selected line
              ...
              (when str
                (let ((start 0) end
                  ...
                  (while (< start (length str))
                    (setq end (or (cl-position ?\n str :start start) (length str)))
                    (when (text-property-any start end 'icomplete-selected t str)
                      (let ((pad (max 0 (- width (string-width (substring str start end))))))
                        (when (> pad 0)
                          (let ((spaces (propertize (make-string pad ?\s) 'face 'hl-line)))
                            (setq str (concat (substring str 0 end) spaces (substring str end))
                                  end (+ end pad))))
                        (add-face-text-property start end 'hl-line nil str)))
                    (setq start (1+ end)))))
              str))
#+end_src
  ...
  (add-hook 'ghostel-mode-hook
            (lambda ()
              (add-hook 'meow-switch-state-hook
                        (lambda (state)
                          (pcase state
                            ('normal (unless (eq ghostel--input-mode 'copy)
                                       (ghostel-copy-mode))
                                     (setopt cursor-type meow-cursor-type-normal))
                            ('insert (when (eq ghostel--input-mode 'copy)
                                       (ghostel-readonly-exit))
                                     (ghostel-semi-char-mode)
                                     (setopt cursor-type meow-cursor-type-insert))))
                        nil t))))
