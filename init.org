...
(setup ghostel
  ...
  (:when-loaded
    (define-key ghostel-char-mode-map (kbd "M-RET") #'ghostel-char-exit-to-normal)
    (define-key ghostel-char-mode-map (kbd "M-<return>") #'ghostel-char-exit-to-normal)
...
;; (setopt eldoc-echo-area-prefer-doc-buffer nil)
#+end_src
***** eglot in org-src
#+begin_src elisp
...
#+begin_src elisp
;;; ─── 分组（基于 Category）────────────────────────
(defun nowis/agenda-entry-category ()
  "返回当前 agenda 行的真实 category：归档条目优先用 ARCHIVE_CATEGORY。"
...
(defun nowis/agenda-cmp-closed (a b)
  "按 CLOSED 时间比较两个 agenda 行（配合 user-defined-down 实现时间倒序）。"
  (cl-flet ((secs (line)
              (let ((m (get-text-property 0 'org-marker line)))
                (if m (with-current-buffer (marker-buffer m)
                        (save-excursion (goto-char m)
                          (let ((c (org-entry-get (point) "CLOSED")))
                            (if c (float-time (org-time-string-to-time c)) 0))))
                  0))))
    (let ((sa (secs a)) (sb (secs b)))
  ...
  `(,key ,(format "近%d天完成" days)
         ((tags ,(format "CLOSED>=\"<-%dd>\"" days)
                ((org-agenda-overriding-header ,(format "近 %d 天完成" days))
                 ...
                 (org-agenda-sorting-strategy '(category-keep user-defined-down)))))))
