...
(transient-define-prefix apps-leader-map()
  ...
  [["agenda"
   ...
   ["voice"
    ...
    ("s r" "sherpa-record" emacs-sherpa-only-record)
...
#+begin_src elisp
(setup project
  (keymap-global-set "C-x p t" #'project-ghostel)
  (:when-loaded
    (add-to-list 'project-vc-extra-root-markers ".project")
    (add-to-list 'project-switch-commands '(project-ghostel "Ghostel" "t"))
    (define-key project-prefix-map "t" #'project-ghostel)))

(defun project-ghostel ()
  "Start ghostel in the current project's root directory. With -u, create a new buffer even if one exists."
  (interactive)
  (let* ((default-directory (project-root (project-current t)))
         (buf-name (project-prefixed-buffer-name "ghostel"))
         (ghostel-buffer-name buf-name))
    (if (or current-prefix-arg (not (get-buffer buf-name)))
        (ghostel)
...
** ghostel
#+begin_src elisp
;; meow insert<->char-mode, normal<->copy-mode.
(defun ghostel-meow-sync (state)
  (pcase state
    ('normal (unless (eq ghostel--input-mode 'copy) (ghostel-copy-mode))
             (setq cursor-type meow-cursor-type-normal))
    ('insert (ghostel-char-mode)
             (setq cursor-type meow-cursor-type-insert))))

(with-eval-after-load 'meow
  (add-hook 'ghostel-mode-hook
            (lambda ()
              (add-hook 'meow-switch-state-hook #'ghostel-meow-sync nil t))))

(defun ghostel-char-exit-to-normal ()
  ;; meow--switch-state (not meow-normal-mode) so the sync hook fires.
  (interactive)
  (when (bound-and-true-p meow-mode) (meow--switch-state 'normal)))

(defun ghostel-edit-input ()
  (interactive)
  (let ((src (current-buffer))
        (buf (generate-new-buffer "*ghostel-edit*")))
    (pop-to-buffer buf)
    (text-mode)
...
(setup ghostel
  ...
  (:when-loaded
    (define-key ghostel-char-mode-map (kbd "M-\\") #'ghostel-char-exit-to-normal)
    (define-key ghostel-char-mode-map (kbd "M-'") #'ghostel-edit-input)
...
1. =inbox.org= for capture inbox TODOs
...
(setup org
  ...
  (defun nowis/gtd-files ()
    "GTD 目录下的活跃 org 文件（排除 beorg/archive）。"
    (let ((dir (nowis/gtd-dir)))
  ...
  (defun org-agenda-update-agenda-files ()
    ...
    (interactive)
    (let ((gtd-files (nowis/gtd-files)))
      (setq org-refile-files (cl-remove-if (lambda (f) (string-match-p "inbox\\|agenda\\|reading" f))
                                           gtd-files))
      (setopt org-agenda-files (cl-remove-if (lambda (f) (string-match-p "incubate" f))
                                           gtd-files))))
  ;; 启动时刷新一次即可
  ...
  (:when-loaded
    (setq org-capture-templates
          `(("i" "Inbox" entry (file "gtd/inbox.org")
             ,(concat "* TODO [#C] %?  :%^{Area|misc|work|grow|health|family|life}:\n"
                      ":PROPERTIES:\n:CREATED: %U\n:END:\n"))
            ("d" "Done" entry (file "gtd/inbox.org")
...
#+begin_src elisp
;;; ─── 分组（基于 Category）────────────────────────
(defun nowis/agenda-entry-category (&optional marker)
  "项目名（分组键）唯一来源：优先 ARCHIVE_CATEGORY，否则普通 category。
MARKER 空=用当前 agenda 行（分隔标题）；非空=跨 buffer 取值（排序）。"
  (if marker
      (with-current-buffer (marker-buffer marker)
        (save-excursion
          (goto-char marker)
          (or (org-entry-get (point) "ARCHIVE_CATEGORY")
              (org-get-category (point)))))
    (let ((m (get-text-property (point) 'org-marker)))
      (or (and m (org-entry-get m "ARCHIVE_CATEGORY"))
...
(defun nowis/agenda-add-category-separators ()
  ...
  (save-excursion
    ...
    (let ((last-category nil))
      (while (not (eobp))
        (when-let* ((cat (nowis/agenda-entry-category)))
...
;;; ─── 近期完成（复用 agenda，临时含 archive，可回车跳转）─────────────────
(defun nowis/recent-done-files ()
  "近期完成扫描范围：活跃文件 + 当年 archive。"
  (cl-remove-if-not
   #'file-exists-p
   (append (nowis/gtd-files)
           (list (concat (nowis/gtd-dir) "/gtd_archive_" (format-time-string "%Y"))))))
...
(defun nowis/agenda-cmp-closed (a b)
  ...
  (let* ((ma (get-text-property 0 'org-marker a))
         ...
         (ca (if ma (nowis/agenda-entry-category ma) ""))
         (cb (if mb (nowis/agenda-entry-category mb) "")))
    (cond
  ...
  `(,key ,(format "近%d天完成" days)
         ((tags ,(format "CLOSED>=\"<-%dd>\"" days)
                ((org-agenda-overriding-header ,(format "近 %d 天完成" days))
                 (org-agenda-files (nowis/recent-done-files))
...
(defun nowis/org-archive-subtree ()
  ...
  (let* ((current-file (buffer-file-name))
         ...
         (archive-file (concat "gtd_archive_" (format-time-string "%Y" completed-time)
                               "::datetree/")))
    (setopt org-archive-location archive-file)
    (org-archive-subtree)))
#+end_src
** org-sm
#+begin_src elisp
...
#+begin_src elisp
(setq org-bookmarks-file (concat nowis/doc-emacs-dir "01-orgmode/xnotes/incremental/20250731T173729--bookmarks.org"))
