...
(setup doom-modeline
  ...
  (:when-loaded
    (custom-set-faces
     ...
     '(mode-line-inactive ((t (:height 0.8))))) ; xs
     )
...
(setup edwina
  (:defer
   (require 'edwina)
   (setq display-buffer-base-action '(display-buffer-in-direction (direction . up)))
...
(setq rime-disable-predicates '(meow-normal-mode-p
         ...
         rime-user-data-dir "~/Documents/emacs/30-input-rime/"
...
(setup dired-preview
  (setq dired-preview-delay 0.3
        dired-preview-trigger-commands '(dired-next-line dired-previous-line)
        )
...
(defun my/zettel-signature-buffer ()
  ...
  (let ((find-ls-option '(" -exec ls -ld {} \\+|sort -t '=' -k 3" . "-ld")))
...
(defun rsync-push-binary-files()
  (interactive)
  (shell-command "rsync -avbihn --exclude '.git' --delete --backup-dir \
  ...
  (let ((answer (read-char-choice "Do you want to continue? [y/n] " '(?y ?n))))
    (cond
     ((eq answer ?y)
      (async-shell-command " rsync -avbih --exclude '.git' --delete --backup-dir \
     ~/Documents/emacs/02-binary-files/ lewis-nas@192.168.31.108:/share/my_webdav/rsync/02-binary-files/"))
     ((eq answer ?n) (message "transfer exit")))))

(defun rsync-pull-binary-files()
  (interactive)
  (async-shell-command " rsync -avbih --exclude '.git' --delete\
