...
(setup meow
  ...
  (defun nowis/surround (char)
    "读一个 CHAR, 用它包围选区。复用 `insert-pair' 的全部能力。"
    (interactive "c包围字符: ")
    (unless (assq char insert-pair-alist)
      (setq-local insert-pair-alist (cons (list char char) insert-pair-alist)))
    (let ((last-command-event char))
      (call-interactively #'insert-pair)))
  (keymap-global-set "M-[" #'nowis/surround)
  )
#+end_src
...
;; 终端里的 `ff'(= ghostel_cmd find-file-other-window)在 tramp 会话下
;; 收到的是裸远程路径;这里把它 tramp 化,让本地/tramp-rpc 两种情况统一。
(defun ghostel-find-file-other-window (path)
  (find-file-other-window (concat (or (file-remote-p default-directory) "") path)))
...
(setup ghostel
  ...
  (:when-loaded
    (setf (alist-get "find-file-other-window" ghostel-eval-cmds nil nil #'equal)
          '(ghostel-find-file-other-window))
...
(defun ghostel-toggle-window (&optional choose-buffer)
  "Toggle ghostel; C-u to create a new buffer or switch to an existing one."
  (interactive "P")
  (cond
   (choose-buffer
    (let* ((names (mapcar #'buffer-name (ghostel--buffers)))
           (name (completing-read "Ghostel buffer (new name to create): " names)))
          ...
          (tags-todo "action"
                     ((org-agenda-overriding-header (nowis/center-header " Ready to Start"))
                      ...
                      (org-agenda-sorting-strategy
                       '(category-up todo-state-up priority-down timestamp-up))))
  ...
  (:when-loaded
    (setq-default gptel-backend
                  (gptel-make-anthropic "momenta"
                    ...
                    :key 'gptel-api-key
                    :stream t
                    :models '(claude-sonnet-5 claude-opus-4.8 gpt-5.6-terra gpt-5.6-luna)))
