...
(transient-define-prefix apps-leader-map()
  ...
  [["agenda"
   ...
   ["capture"
    ...
    ("e" "extract topic" nowis/org-sm-topic-capture-from-clipboard)
    ("E" "extract topic with prefix"
     (lambda () (interactive) (nowis/org-sm-topic-capture-from-clipboard '(-1))))
    ]
   ["life"
...
(defvar nowis/org-sm-topic-capture-file
  (concat nowis/doc-emacs-dir "01-orgmode/xnotes/org-inc/20251201T112101--incubate.org")
  "Target file for org-sm topic capture.")
    ...
    (setq org-capture-templates
          `(("i" "Inbox" entry (file "gtd/inbox.org")
            ...
            ("d" "Done without clock" entry (file+olp+datetree "gtd/inbox.org")
             ,(concat "* DONE %?\n" "CLOSED: %U"))
            ("t" "org-sm topic" entry (file nowis/org-sm-topic-capture-file)
             ,(concat "** topic card\n" ":PROPERTIES:\n:CREATED: %U\n:END:\n%?")
             :before-finalize (lambda () (org-sm-item-mark 'topic))))))

  (defun nowis/org-sm-topic-capture-from-clipboard (&optional arg)
    "Capture a topic from clipboard or active region.
With prefix ARG, prompt to change the target file first."
    (interactive "P")
    (when arg
      (setq nowis/org-sm-topic-capture-file
            (read-file-name "Topic capture file: " org-sm-directory nil t nil
...
(setup org-sm
  (:hook-into org-mode)
  (setq org-sm--files (directory-files-recursively
                       (expand-file-name "org-inc/" org-directory)
                       "[^/]*srs[^/]*\\.org$"
                       ))
...
(defun nowis-capture-inc ()
  ...
  (sit-for 0.1)
  (nowis/org-sm-topic-capture-from-clipboard))
