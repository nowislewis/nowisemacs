...
#+begin_src elisp :tangle no
  (setup (:pkg awesome-pair :host github :repo "manateelazycat/awesome-pair")
    ...
    (:bind
     ;; (kbd "(") awesome-pair-open-round
     ;; (kbd "[") awesome-pair-open-bracket
     ;; (kbd "{") awesome-pair-open-curly
     (kbd ")") awesome-pair-close-round
     (kbd "]") awesome-pair-close-bracket
     (kbd "}") awesome-pair-close-curly
     ;; (kbd "=") 'awesome-pair-equal
     (kbd "%") awesome-pair-match-paren
     (kbd "\"") awesome-pair-double-quote
     ;; (kbd "SPC") awesome-pair-space
     ;; (kbd "RET") awesome-pair-newline
...
#+begin_src elisp
(setup (:pkg vundo :host github :repo "casouri/vundo"))
#+end_src
*** undo-hl flashes the to-be-deleted text before deleting
#+begin_src elisp
  (setup (:pkg undo-hl :host github :repo "casouri/undo-hl")
...
#+end_src
*** conda
don't use conda any more for a better integration with EAF
#+begin_src elisp :tangle no
  (setup (:pkg conda)
    ;; (:delay)
    (:when-loaded
...
#+end_src
*** project
borrowed from https://emacstalk.github.io/post/010/
#+begin_src elisp :tangle no
...
*** delve
#+begin_src elisp
(setup (:pkg delve :host github :repo "publicimageltd/delve"))
#+end_src
...
#+begin_src elisp :tangle no
  (setup (:pkg mpv))
  (mpv start)
  (setup (:pkg pretty-hydra))
  (setup (:pkg org-media-note :host github :repo "yuchen-lea/org-media-note")
    (:option org-media-note-screenshot-image-dir "~/Documents/emacs/orgmode/PicturesForAll/org_media_imgs/")
    (:autoload org-media-note-hydra/body)
    )
    #+end_src
* 翻译
** go-translate
...
#+begin_src elisp
  ...
  ;;   (:option book-scroll-fraction 16)
  ;;   )
  ;; (defun load-pdf-continuous-mode()
  ;;   (interactive)
...
#+end_src
** ebib
ebib doesn't work well with vertico now, so tangle no at this time.
#+begin_src elisp
  (setup (:pkg ebib)
    (:option ebib-default-directory "~/Documents/emacs/orgmode/bibliography/"
