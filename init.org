...
;; advice 将其改回 orderless。
(advice-add 'icomplete--fido-mode-setup :after
            (lambda ()
              (setq-local completion-styles '(orderless basic))))

;; orderless 依赖空格
(define-key icomplete-fido-mode-map " " #'self-insert-command)
...
(advice-add 'icomplete--render-vertical :around
            (lambda (orig comps md)
              (cl-letf (((symbol-function 'make-string)
                         (lambda (n c) (if (= c 32) " " (make-string n c)))))
                (funcall orig comps md))))
#+end_src
