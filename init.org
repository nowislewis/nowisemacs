...
└── 09-scripts
#+end_example
1. All files and folders except =02-binary-git/binary-files= are managed under git control, and =02-binary-file= is managed under =rsync= or =restic=.
...
(transient-define-prefix lewis-leader-map()
  ...
  [
   ...
   ["gptel-aibo"
    ("a" "aibo" gptel-aibo)
    ("c" "complete-at-point" gptel-aibo-complete-at-point)
    ]
   ;; org-download
   ;; ["Org download"
...
*** dictionary
#+begin_src elisp
(transient-define-prefix dictionary-leader-map()
  "Define leader-key map for ui functions"
  [["dictionary-overlay"
   ("s" "start" dictionary-overlay-start)
   ("r" "render-toggle" dictionary-overlay-toggle)
   ("m" "mark-word-unknown" dictionary-overlay-mark-word-unknown)
   ("M" "mark-word-known" dictionary-overlay-mark-word-known)
   ("i" "install" dictionary-overlay-install)
   ("R" "restart" dictionary-overlay-restart)
   ]])
#+end_src
...
** org-pipeline
#+begin_src elisp :tangle no
(defun org-babel-execute:pipeline (body params)
  (let* ((script (cdr (assoc :script params)))
         (in-file (make-temp-file "org-input-"))
         (out-file (make-temp-file "org-output-"))
         (cmd (format "cat %s | %s > %s"
                     (shell-quote-argument in-file)
                     script
                     (shell-quote-argument out-file))))

    (write-region body nil in-file nil 'silent)
    (message cmd)
    (shell-command cmd)
      (with-temp-buffer
...
(setup org-download
  (setq org-download-method 'directory
           ...
           org-download-image-dir (concat nowis-doc-emacs-dir "02-binary-git/binary-files/01-pictures/org_download_images")
...
#+begin_src elisp
(setup org-noter
  (setq org-noter-notes-search-path (concat nowis-doc-emacs-dir "02-binary-git/binary-files/02-searchable/01-papers-markable/")
...
(setup org-excalidraw
   (setq org-excalidraw-default-directory (concat nowis-doc-emacs-dir "05-excalidraw/"))
  ...
  `(list ,key ,description
         '((agenda ""
           ...
           (tags-todo ,(concat "+action" tag) ((org-agenda-overriding-header "Action")
                                               (org-agenda-prefix-format "%-12:c [%-4e] ")
...
(setup dictionary-overlay
  (:autoload dictionary-overlay-start
  ...
  (setq dictionary-overlay-user-data-directory (concat nowis-doc-emacs-dir "12-dictionary-overlay")
        dictionary-overlay-lookup-with #'gt-quick-sdcv
        ;; dictionary-overlay-translators '("local" "sdcv" "darwin" "web")
        dictionary-overlay-sdcv-dictionary-path (concat nowis-doc-emacs-dir "02-binary-git/binary-files/03-stardicts/stardict-ecdict-2.4.2/stardict-ecdict-2.4.2")
        ))
  ...
  (:when-loaded
    (setq-default gptel-backend
                  (gptel-make-openai "chatanywhere"
                    ...
                    :key 'gptel-api-key
                    :stream t
                    :models '(gpt-4o-mini deepseek-reasoner gemini-1.5-flash-latest)))
