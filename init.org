...
;;; -*- lexical-binding: t; -*-
(setopt gc-cons-threshold 128000000
...
(transient-define-prefix buffer-leader-map()
  ...
  [["Buffer Switch"
    ...
    ("f" "buffer-other-frame" consult-buffer-other-frame)
    ("w" "buffer-other-window" consult-buffer-other-window)
...
(transient-define-prefix apps-leader-map()
  ...
  [["agenda"
    ...
    ("g" "GTD workflow" nowis/gtd-workflow-menu)
   ...
   ["capture"
    ...
    ("e" "capture (org-sm)" org-sm-capture)
    ]
   ["voice"
    ("v d" "sherpa-dictate" emacs-sherpa-dictate)
    ("v r" "sherpa-only-record" emacs-sherpa-only-record)
    ("v f" "sherpa-dictate-file" emacs-sherpa-dictate-file)
    ("v k" "kokoro-read" emacs-kokoro-read)
    ]
   ["others"
    ("r" "restart emacs" restart-emacs)
    ("s" "desktop save" desktop-save-in-desktop-dir)
    ("l" "desktop read" desktop-read)
    ]
   ])
#+end_src
*** ui
...
;; buffer
 ...
 '("TAB" . tab-leader-map)
 '("t" . toggle-leader-map)
 ;; org-sm: direct review start and discoverable command menu
...
(defun nowis/setup-fonts()
  ...
  (when (display-graphic-p)
    (when lewis-fixed-font
        (set-face-attribute 'default nil :family lewis-fixed-font :height lewis-font-size)
        (set-face-attribute 'fixed-pitch nil :family lewis-fixed-font :height lewis-font-size))
...
(setup consult
  ...
  ;; 选 buffer 时自动跳到它所在的 tab（替代 tabspaces-switch-buffer-and-tab）
  (with-eval-after-load 'consult
    ;; 选 buffer 时：若它属于别的 tab，先把当前窗口(被 preview 污染)切回
    ;; 原来的 buffer，再跳到目标 tab 显示。这样当前 tab 不残留该 buffer。
    (plist-put
     consult-source-buffer :action
     (lambda (buf)
       (if-let* ((tab (tab-bar-get-buffer-tab buf nil t))
                 (name (alist-get 'name tab)))
           (progn
             ;; 恢复当前窗口：切回 preview 之前显示的 buffer
             (when-let* ((prev (caar (window-prev-buffers))))
               (switch-to-buffer prev 'norecord))
...
(keymap-global-set "M-i" #'completion-help-at-point)
...
(setup completion-preview
  ...
  (:when-loaded
    ;; Org remaps ordinary insertion/deletion commands
...
;; shadow unused file path
(setopt file-name-shadow-properties '(invisible t intangible t field shadow))
;; Reuse fido-mode's own DEL-to-updir
(require 'icomplete)
(keymap-set minibuffer-local-filename-completion-map
            "DEL" #'icomplete-fido-backward-updir)

(defun nowis/minibuffer-kill-ring-save ()
  "Copy the region if active, else the whole minibuffer input."
  (interactive)
  (if (use-region-p)
      (kill-ring-save (region-beginning) (region-end))
    (kill-new (minibuffer-contents))
    (message "Copied: %s" (minibuffer-contents))))
(keymap-set minibuffer-mode-map "M-w" #'nowis/minibuffer-kill-ring-save)
#+end_src
...
(setup embark
  ...
  (:when-loaded
    ;; fix completion buffer embark-act insert, rm end of work
    (setf (alist-get 'embark-insert embark-pre-action-hooks) nil)))
...
(with-eval-after-load 'rime
  ;; Override: use \cC (real CJK only) to include ─ │ ┌ etc(mindmap)
...
#+begin_src elisp
(setup speedbar
  (:bind "TAB" speedbar-toggle-line-expansion)
...
#+begin_src elisp
(setup desktop
  (setopt desktop-dirname user-emacs-directory
          desktop-restore-eager 8                 ; 立即只恢复 8 个可见 buffer,其余等切到时才读
          desktop-lazy-idle-delay 5               ; 空闲 5s 再后台补齐剩余 buffer
          desktop-lazy-verbose nil                ; 后台恢复别刷屏
          desktop-files-not-to-save "\\(^/[^/:]*:\\|(ftp)$\\)"   ; 不碰远程
          desktop-modes-not-to-save '(eshell-mode term-mode ghostel-mode comint-mode
                                                  tags-table-mode dired-mode magit-status-mode
                                                  magit-status-mode
                                                  magit-revision-mode
                                                  magit-diff-mode
                                                  magit-process-mode)
...
(setup ghostel
  ...
  (add-to-list 'load-path
               (expand-file-name "lib/ghostel/extensions/consult-ghostel"
                                 user-emacs-directory))
  (autoload #'nowis-ghostel "nowis-ghostel" nil t)
  (keymap-global-set "M-`" #'nowis-ghostel)
  (:when-loaded
    (require 'nowis-ghostel)
    (nowis-ghostel-setup)))
#+end_src
* git
** magit
...
**** ratex
#+begin_src elisp
(setup ratex ;; emacs must be built with librsvg
...
(setup cape
  ...
  (setq cape-dict-file
   ...
   (defalias 'my/cape-dabbrev-dict
     (cape-capf-sort (cape-capf-super #'cape-dabbrev #'cape-dict)))
  ;; `add-hook' without LOCAL updates the global default, which Org's
  ;; buffer-local CAPF list inherits through its `t' marker.
  (dolist (f (list #'my/cape-dabbrev-dict #'cape-elisp-block #'cape-file))
    (add-hook 'completion-at-point-functions f)))
#+end_src
**** lsp mode
***** eglot
#+begin_src elisp
...
(setup jit-spell
  ...
  (:when-loaded
    (keymap-set jit-spell-mode-map "M-$" #'jit-spell-correct-word)
    (keymap-global-set "C-M-$" #'jit-spell-change-dictionary)))
#+end_src
*** debug
**** gdb
#+begin_src elisp
...
(setup org
  ...
  (setopt org-startup-indented t
          ...
          org-image-actual-width 800 ;; this will use 800 for width for all the images.
...
(with-eval-after-load 'org
  (setf (alist-get 'tectonic org-preview-latex-process-alist)
        '(:programs ("tectonic" "convert")
                    :description "pdf > transparent png"
                    ...
                    :image-converter
                    ("convert -density %D -background none -alpha on %f -trim +repage -quality 300 %O")))
  (setopt org-preview-latex-default-process 'tectonic)
...
(setup org
  (:when-loaded
    ...
    (defconst nowis/org-heading-stars
      ["󰯭" "󰯰" "󰯳" "󰯶" "󰯹" "󰯼"
       "󰯿" "󰰂" "󰰅" "󰰈" "󰰋"])
    ...
    (font-lock-add-keywords
     'org-mode
     `(("^\\(\\*+\\) "
        (1 (let ((end (match-end 1)))
             (compose-region
              (1- end) end
              (aref nowis/org-heading-stars
                    (min (1- (- end (match-beginning 1))) 10)))
             nil)))))
    ))
;; ┌───────────────┐
;; │ prettify-mode │
...
(setopt org-enforce-todo-dependencies t
        org-log-done 'note
        org-todo-keywords '((sequence "TODO(t)" "NEXT(n)" "WAIT(w)" "▶(p@)" "|" "AXED(a)" "DONE(d)"))
        )
#+end_src
*** capture
#+begin_src elisp
...
*** agenda
#+begin_src elisp
;;; ─── 跳到任务所属 project（看 note）──────────────────────
(defun nowis/agenda-goto-project ()
  "从 agenda 当前任务跳到它所属的 project heading（level 2），看该战线的 note。"
  (interactive)
  (org-agenda-goto)                     ; 复用原生跳转（开窗、widen、展开都处理好）
  (while (> (org-current-level) 2)      ; 上溯到 project 层
    (org-up-heading-safe))
  (org-fold-show-children)
  (recenter-top-bottom 2))

(with-eval-after-load 'org-agenda
  (keymap-set org-agenda-mode-map "P" #'nowis/agenda-goto-project))
...
;;; ─── Agenda 视图构建 ────
;; blocked 任务在 agenda 中隐藏
(setq org-agenda-dim-blocked-tasks 'invisible)

;;; GTD 主视图（g）；近期完成回顾（r)
(setq org-agenda-custom-commands
      '(("r" "近 N 天完成（按 oldpath）"
         (lambda (_) (call-interactively #'nowis/review-done-by-project)))

        ("g" "GTD"
         ((agenda ""
                  ((org-agenda-overriding-header (nowis/center-header " Only One at A Time "))
                   (org-agenda-span 'day)
                   (org-agenda-skip-function
                    '(org-agenda-skip-if nil '(todo done)))))

          (todo "NEXT"
                     ((org-agenda-overriding-header (nowis/center-header " Next "))))
          ...
          (tags-todo "action"
                     ((org-agenda-overriding-header (nowis/center-header " Ready to Start"))
                      ...
                      (org-agenda-sorting-strategy
                       '(priority-down timestamp-up todo-state-up category-up))))
...
id 仅作兜底。"
  (when-let* ((closed (org-entry-get nil "CLOSED"))
    ...
    (let* ((olp (org-entry-get nil "ARCHIVE_OLPATH"))
           ...
           (date (substring closed 1 11))
           (file (buffer-file-name (buffer-base-buffer)))
           (id (org-id-get))
           ;; 优先 file+标题搜索（datetree 归档也能命中），id 兵底
           (link (cond
                  (file (format "[[file:%s::%s][%s]]"
                                file (org-link-heading-search-string title) title))
                  (id   (format "[[id:%s][%s]]" id title))
                  (t    title))))
      (cons segs (format "- [%s] %s" date link)))))

(defun nowis/review-done-by-project (days)
  "扫描近 DAYS 天所有 DONE，按完整 oldpath 层级建树，生成只读 org buffer。"
  (interactive "n近多少天完成: ")
  (let* ((match (format "TODO=\"DONE\"+CLOSED>=\"<-%dd>\"" days))
         (items (delq nil (org-map-entries #'nowis/review--entry match
                                           (nowis/gtd-files))))
         ;; 按路径排序，使相同前缀相邻 → 能共享标题
    ...
    (with-current-buffer (get-buffer-create "*Review by Project*")
      (let ((inhibit-read-only t))
        (erase-buffer)
        (org-mode)
        (insert (format "#+title: 近 %d 天完成(按 oldpath)\n\n" days))
        (dolist (item items)
          (let* ((segs (car item))
                 (common 0))
            ...
            (cl-loop for d from common below (length segs) do
                     (insert (make-string (1+ d) ?*) " " (nth d segs) "\n"))
            (setq last segs))
          ;; 完成行缩进到路径深度下一级
...
(setup org-sm
  ...
  (setq org-sm-directory (expand-file-name "incremental/" org-directory)
        org-sm-capture-file
        (concat nowis/doc-emacs-dir
                "01-orgmode/xnotes/incremental/"
                "20260331T230205--incremental-reading-materials.org"))
  ;; Extract selected text; otherwise preserve M-z as the global capture key.
  (nowis/smart-bind "M-z" #'region-active-p #'org-sm-item-extract)
  (:when-loaded
    ;; AI only refines existing cards in the workbench; capture stays raw.
    (require 'org-sm-gptel))
  (transient-define-prefix org-sm-transient-map ()
    "Discover org-sm commands; review continuation uses the repeat map."
    [["review"
      ("r" "review confirm" org-sm-review-confirm)
      ("s" "start review light" org-sm-review-start-light)
      ("S" "start review" org-sm-review-start)
      ("a" "about" org-sm-review-abort)
      ("l" "tree" org-sm-tree)
      ("c" "capture" org-sm-capture)
      ("e" "capture clipboard" org-sm-capture-clipboard-notify)]
     ["card"
      ("m" "mark" org-sm-item-mark)
      ("z" "extract selection" org-sm-item-extract)
      ("d" "dismiss" org-sm-item-dismiss)
      ("u" "undismiss" org-sm-item-undismiss)
      ("p" "set read point" org-sm-set-readpoint)
      ("o" "goto source" org-sm-goto-source)]
     ["read"
      ("z" "extract selection" org-sm-item-extract)
      ("p" "set read point" org-sm-set-readpoint)
      ("o" "goto source" org-sm-goto-source)]
     ["AI"
      ...
      ("w" "workbench" org-sm-card-workbench)]]))
#+end_src
...
#+begin_src elisp
(setup org-mindmap
(setq org-mindmap-default-max-width nil)
...
(setup gptel
  ...
  (:when-loaded
    ...
    (gptel-make-openai "momenta-openai"
      :host "llm-gateway.momenta.works"
      :key 'gptel-api-key
      :stream t
      :models '(glm-5.3 qwen3.8-flash))
    ...
    (dolist (model '(claude-opus-5 claude-sonnet-5 gemini-3.8-flash qwen3.8-flash))
      (put model :capabilities '(media tool-use cache)))
    )
  )
#+end_src
*** gptel-agent
...
(setup gptel-agent
  ...
  (:when-loaded
    ...
    (require 'gptel-robust)
))
#+end_src
*** gptel-preset-collection
#+begin_src elisp
(setup gptel-preset-collection
...
#+begin_src elisp
(setup gptel-inline
  (keymap-global-set "M-l" #'gptel-inline)
  (:when-loaded
    (define-advice gptel-inline--response-overlay-append-chunk
        (:around (fn ov chunk) inhibit-read-only)
      (let ((inhibit-read-only t))
        (funcall fn ov chunk)))
    ))
#+end_src
...
#+begin_src elisp
(setup nowis-review
  (keymap-global-set "M-m" #'nowis-review-mark))
#+end_src
** webjump
#+begin_src elisp
(setup webjump
...
(unless (display-graphic-p)
  ...
  (require 'term/screen)
  (add-to-list 'xterm-screen-extra-capabilities 'setSelection)
...
(transient-define-prefix nowis-popup--transient ()
  ...
  [
   ...
   ["org-sm"
    ("r" "start review" org-sm-review-start)
    ("l" "tree" org-sm-tree)
    ("e" "capture clipboard" nowis-popup-org-sm-capture-clipboard)]
...
** nethack
#+begin_src elisp
(defun nowis-nethack-disable-meow ()
  "Let NetHack own all input keys in its interactive buffers."
  (meow-mode -1))

(setup nethack
  (setq nethack-use-tiles "nethack500")
  (add-hook 'nethack-map-mode-hook #'nowis-nethack-disable-meow)
  (add-hook 'nethack-menu-mode-hook #'nowis-nethack-disable-meow))
#+end_src
* system-config
** wslg
#+begin_src elisp
;; Work around Emacs 32 PGTK clipboard issues under WSLg.
(when (and (getenv "WSL_DISTRO_NAME")
           (eq window-system 'pgtk)
           (executable-find "wl-copy")
           (executable-find "wl-paste"))
  (defun nowis/wslg-clipboard-copy (text)
    "Copy TEXT to WSLg as UTF-8."
  ...
  (defun nowis/wslg-clipboard-paste ()
    "Return new UTF-8 text from the WSLg clipboard."
    (let ((text (with-temp-buffer
                  (let ((coding-system-for-read 'utf-8-unix))
                    (when (zerop (call-process "wl-paste" nil t nil "-n" "-t"
                                               "text/plain;charset=utf-8"))
                      (buffer-string))))))
      (unless (or (string-empty-p (or text ""))
                  (and kill-ring (equal text (substring-no-properties
                                              (car kill-ring)))))
        text)))

  (setq interprogram-cut-function #'nowis/wslg-clipboard-copy
        interprogram-paste-function #'nowis/wslg-clipboard-paste))
#+end_src
