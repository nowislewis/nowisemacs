...
;;; 辅助函数
    (defvar doc-capture-org-inc--title "helo"
      "临时存储标题")

    (defvar doc-capture-org-inc--content "content"
      "临时存储内容")

    (defun doc-capture-org-inc-get-title ()
      "获取标题"
      doc-capture-org-inc--title)

    (defun doc-capture-org-inc-get-content ()
      "获取内容"
      doc-capture-org-inc--content)
    (defcustom doc-capture-org-inc-file (expand-file-name "~/org-inc.org")
      "org-inc 数据库文件路径"
      :type 'file
      :group 'doc-capture)
          ...
          `(("i" "Inbox" entry  (file+olp+datetree "gtd/inbox.org")
            ...
            ("d" "Done without clock" entry (file+olp+datetree "gtd/inbox.org")
             ,(concat "* DONE %?\n"
                      "CLOSED: %U"))
            ("t" "org-inc topic"
             entry (file ,doc-capture-org-inc-file)
             "* %(doc-capture-org-inc-get-title)\n%(doc-capture-org-inc-get-content)"
             :immediate-finish t
             ;; :jump-to-captured t
             :hook ,(lambda ()
...
(setup gptel
  ...
  (:when-loaded
    ...
    (gptel-make-anthropic "momenta"
      ...
      :key 'gptel-api-key
      :stream t
      :models '(claude-sonnet-4.5 claude-haiku-4.5 minimax-m2 gemini-3-pro deepseek-v3.2))
